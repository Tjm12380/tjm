    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'" label="商品名称" prop="shangpinmingcheng">
          <el-select :disabled="ro.shangpinmingcheng" @change="shangpinmingchengChange" v-model="ruleForm.shangpinmingcheng" placeholder="请选择商品名称">
            <el-option
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `kehutuihuo/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.kehutuihuoCrossAddOrUpdateFlag = false;
						 this.parent.search();
