...
export default {
  data() {
	...
	shhf : false,
      ...
      rules: {
          cangguanzhanghao: [
                { required: true, message: '仓管账号不能为空', trigger: 'blur' },
          ],
          mima: [
                { required: true, message: '密码不能为空', trigger: 'blur' },
          ],
          cangguanxingming: [
                { required: true, message: '仓管姓名不能为空', trigger: 'blur' },
          ],
          xingbie: [
          ],
          shouji: [
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `cangguan/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.cangguanCrossAddOrUpdateFlag = false;
						 this.parent.search();
